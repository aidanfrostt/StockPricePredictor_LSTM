"""
Enhanced Stock Prediction System with All Improvements
Author: AI Assistant
Description: Incorporates all suggested improvements for better predictions
"""
...
def fetch_stock_data(ticker, period=CONFIG['DATA']['HISTORY_PERIOD'], interval=CONFIG['DATA']['INTERVAL']):
    """Fetch enhanced historical stock data with news sentiment"""
    try:
        # Fetch stock data
...
def preprocess_data(data):
    """Enhanced data preprocessing with all suggested features"""
    if data is None or data.empty:
        return None
        
    print(f"Raw data shape before preprocessing: {data.shape}")
    ...
    try:
        ...
        data['bb_position'] = (data['Close'] - bb.bollinger_lband()) / \
        ...
        print(f"Error adding custom features: {str(e)}")
...
def build_lstm_model(input_shape):
    """Enhanced LSTM model with suggested architecture"""
    try:
        model = Sequential([
...
def prepare_rf_data(data, target_col='Close'):
    """Enhanced Random Forest data preparation with suggested features"""
    try:
        data = data.copy()
        ...
        print(f"Error preparing RF data: {str(e)}")
        return None, None

def train_random_forest(X, y):
    """Improved Random Forest training with feature importance analysis"""
    try:
        # Feature selection
...
def adjust_predictions(predictions, rsi, bb_position):
    """Apply mean-reversion adjustments based on technical indicators"""
    try:
...
def prepare_lstm_data(data, target_col='Close', n_steps=60, test_size=0.2):
    """Prepare data for LSTM with consistent feature selection"""
    try:
        # Select only numeric features and exclude target
        numeric_cols = data.select_dtypes(include=[np.number]).columns.tolist()
        feature_cols = [col for col in numeric_cols if col != target_col]
        
        # Ensure we always include these key features
...
def predict_with_lstm(model, data, target_scaler, feature_cols, n_steps=60, days_to_predict=30):
    """Make predictions using trained LSTM model with exact feature matching"""
    try:
        # Verify and prepare feature columns
        if not isinstance(feature_cols, list):
            raise ValueError("feature_cols must be a list")
            
        available_cols = [col for col in feature_cols if col in data.columns]
        if len(available_cols) != len(feature_cols):
            print(f"Warning: {len(feature_cols)-len(available_cols)} features missing from prediction data")
        
        # Ensure Close is included for prediction updates
        if 'Close' not in available_cols and 'Close' in data.columns:
            available_cols.append('Close')
        
        # Scale features using the same scaler approach as training
        feature_scaler = RobustScaler()
        ...
        for _ in range(days_to_predict):
            ...
            # Update sequence with predicted close price
        ...
        bb = BollingerBands(data['Close'])
        current_bb_pos = (data['Close'].iloc[-1] - bb.bollinger_lband().iloc[-1]) / \
        ...
        print(f"Error making LSTM predictions: {str(e)}")
        return None

def predict_with_random_forest(model, selected_features, data, days_to_predict=30):
    """Enhanced Random Forest prediction with proper feature propagation"""
    try:
        ...
        if missing_features:
            raise ValueError(f"Missing required features: {missing_features[:5]}...")  # Show first 5 missing
        
        last_date = data.index[-1]
        future_dates = [last_date + timedelta(days=i) for i in range(1, days_to_predict+1)]
        
        # Initialize prediction dataframe with the last available data point
        ...
        for i in range(days_to_predict):
            ...
            # Update features for next steps if we're not on the last prediction
            if i < days_to_predict - 1:
                # Update lag features
                for lag in range(1, 4):  # Update lags up to 3 days
                    ...
                    if lag_col in selected_features:
                        if lag == 1:
                            ...
                            if prev_lag_col in selected_features:
                                future_data.loc[future_dates[i+1], lag_col] = \
                                    future_data.loc[future_dates[i], prev_lag_col]
                ...
                for window in [7, 20, 50]:
                    ...
                    if ma_col in selected_features:
                        if len(close_predictions) >= window:
                            future_data.loc[future_dates[i+1], ma_col] = \
                                np.mean(close_predictions[-window:])
                        else:
                            # Not enough predictions yet, use historical data
                            hist_data = data['Close'].iloc[-(window-len(close_predictions)):].tolist()
                            future_data.loc[future_dates[i+1], ma_col] = \
                                np.mean(hist_data + close_predictions)
        
        # Get current technical indicators for adjustment
        current_rsi = RSIIndicator(data['Close']).rsi().iloc[-1]
        bb = BollingerBands(data['Close'])
        current_bb_pos = (data['Close'].iloc[-1] - bb.bollinger_lband().iloc[-1]) / \
                        (bb.bollinger_hband().iloc[-1] - bb.bollinger_lband().iloc[-1])
        ...
        print(f"Error making RF predictions: {str(e)}")
        import traceback
        traceback.print_exc()
        return None
...
def plot_historical_with_predictions(data, predictions):
    ...
    try:
        ...
        if predictions is not None:
            if 'LSTM_Prediction' in predictions.columns:
                plt.plot(predictions['Date'], predictions['LSTM_Prediction'], 
                         label='LSTM Prediction', color='red', linestyle='--', linewidth=2)
            
            if 'RF_Prediction' in predictions.columns:
                plt.plot(predictions['Date'], predictions['RF_Prediction'], 
                         label='Random Forest Prediction', color='green', linestyle='--', linewidth=2)
            
            if 'Average_Prediction' in predictions.columns:
                plt.plot(predictions['Date'], predictions['Average_Prediction'], 
                         label='Ensemble Prediction', color='purple', linestyle='-', linewidth=3)
        
        # Highlight prediction start point
        plt.scatter(data.index[-1], data['Close'].iloc[-1], 
                    color='black', s=150, label='Prediction Start', zorder=5)
        ...
        print(f"Error generating historical plot: {str(e)}")

def plot_technical_indicators(data):
    """Enhanced technical indicators plot"""
    try:
        ...
        try:
            ...
            axes[2].bar(data.index, macd.macd_diff(), 
                       label='MACD Histogram', color=np.where(macd.macd_diff() > 0, 'g', 'r'))
        ...
        try:
            ...
            if 'Volume_MA_20' in data.columns:
                axes[3].plot(data.index, data['Volume_MA_20'], 
                            label='20-day Volume MA', color='blue', linewidth=2)
        ...
        print(f"Error generating technical indicators plot: {str(e)}")
...
def plot_model_history(history):
    ...
    try:
        ...
        print(f"Error generating model history plot: {str(e)}")

# ======================
# MAIN FUNCTION
# ======================

def analyze_stock(ticker, days_to_predict=30):
    """Enhanced main function to analyze and predict stock performance"""
    print(f"\n{'='*50}")
    ...
    if data is None or data.empty:
        print(f"Error: No usable data after preprocessing for ticker '{ticker}'.")
        return None

    last_data_date = data.index[-1].strftime('%Y-%m-%d')
    print(f"Analyzing {ticker} - Last available data: {last_data_date}")
    print(f"{'='*50}")
    ...
    try:
        X_train, X_test, y_train, y_test, target_scaler, feature_cols = prepare_lstm_data(
            data, 
            n_steps=CONFIG['MODEL']['LSTM_LOOKBACK']
        )
    ...
    # Display predictions
    print("\n🔮 Price Predictions:")
    print(predictions.head(days_to_predict))
...
def main():
    """Main user interface"""
    print("📈 Advanced Stock Market Analysis and Prediction System")
    print("----------------------------------------------------")
